...
class EditarUsuarioController(ABC):
    ...
    def __validar_campos(self, usuario_editado) -> None:
        try:
            ValidadorInputs.validar_id(usuario_editado)

            if("Nome" in usuario_editado):
                ValidadorInputs.validar_nome(usuario_editado)

            if("Email" in usuario_editado):
                 ValidadorInputs.validar_email(usuario_editado)

            if("Username" in  usuario_editado):
                ValidadorInputs.validar_username(usuario_editado)
        except ValueError as erro:
